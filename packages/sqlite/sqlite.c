...
DECLARE_MODULE_METHOD(sqlite__open) {
  ...
  b_obj_ptr *ptr = (b_obj_ptr*)GC(new_ptr(vm, th, (void*)db));
  ptr->name = "<SQLite3 *>";
  RETURN_OBJ(ptr);
}
...
DECLARE_MODULE_METHOD(sqlite__query) {
  ...
  if(db != NULL) {
    ...
    if(sqlite3_prepare_v2(db, query->chars, query->length, &stmt, 0) == SQLITE_OK) {
      ...
      b_obj_ptr *ptr = (b_obj_ptr*)GC(new_ptr(vm, th, (void*)stmt));
  ...
  if(stmt != NULL) {
    ...
    switch (col_type) {
      ...
      case SQLITE_BLOB: {
        ...
        b_obj_bytes *bytes = (b_obj_bytes*)GC(new_bytes(vm, th, sizeof(data)));
